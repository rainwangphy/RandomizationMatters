...
class Mixture_of_Classifier(nn.Module):
    ...
    def load(self, top_accuracy_under_attack=False, level=0):
        ...
        for i in range(self.number_of_models):
            ...
            classifier, accuracy, accuracy_under_attack = models.load_model(self.save_dir, i, -1, self.device,
                                                                            self.config['number_of_class'],
                                                                            top_acc_under_attack=top_accuracy_under_attack)
            print((level + 1) * "   " + "Model n°" + str(
                i) + " loaded. Accuracy : %.2f - Accuracy Under Attack : %.2f " % (accuracy, accuracy_under_attack))
            ...
            if not os.path.exists(
                    self.save_dir + "/model_" + str(i) + "/epoch_" + str(self.config['epochs'] - 1) + ".pth"):
                print((level + 1) * "   " + "Classifier not completely trained")
                for j in range(200):
                    if not os.path.exists(self.save_dir + "/model_" + str(i) + "/epoch_" + str(j) + ".pth"):
                        classifier, accuracy, accuracy_under_attack = models.load_model(self.save_dir, i, j - 1,
                                                                                        self.device,
                                                                                        self.config['number_of_class'],
                                                                                        top_acc_under_attack=top_accuracy_under_attack)
                        ...
                        classifier, accuracy, accuracy_under_attack = models.load_model(self.save_dir, i, j,
                                                                                        self.device,
                                                                                        self.config['number_of_class'],
                                                                                        top_acc_under_attack=top_accuracy_under_attack)
    ...
    def trainBoostedClassifier(self, classifier, level=0):
        ...
        optimizer = optim.SGD(classifier.parameters(), lr=lr, momentum=self.config['momentum'],
                              weight_decay=self.config['weight_decay'])
        scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=milestones, gamma=self.config['gamma'],
                                             last_epoch=-1)
        ...
        if id_classifier == 0:
            ...
            for epoch in range(start, self.config['epochs']):
                ...
                models.adversarialTrain(classifier, self.device, self.train_loader, optimizer, epoch, attack,
                                        level=level + 1)
                ...
                accuracy_under_attack = models.test_under_attack(classifier, self.device, self.test_loader, attack,
                                                                 level=level + 1)
                accuracy = models.test(classifier, self.device, self.test_loader, level=level + 1)

                models.save_model(self.save_dir, id_classifier, self.device, classifier, accuracy,
                                  accuracy_under_attack, epoch, level=level + 1)

                models.updateAndSaveBestAccuracies(self.save_dir, id_classifier, self.device, classifier, accuracy,
                                                   accuracy_under_attack, level=level + 1)
        ...
        classifier, acc, top_acc_under_attack = models.load_model(self.save_dir, id_classifier, -1, self.device,
                                                                  self.config['number_of_class'],
    ...
    def test_mixture_accuracy_under_attack(self, adversary="pgd", number_of_models=None, level=0):
        ...
        accuracy = models.test_under_attack(self, self.device, self.test_loader, mixed_attack, adversary=adversary,
                                            level=level + 1)
    ...
    def test_classifier_accuracy_under_attack(self, i, adversary="pgd", level=0):
        ...
        accuracy = models.test_under_attack(self.classifiers[i], self.device, self.test_loader, attack,
                                            adversary=adversary, level=level + 1)
