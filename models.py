...
def train(model, device, train_loader, optimizer, epoch, level=0):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        ...
        print(level * "   " + 'Loss: %.3f | Acc: %.3f%% (%d/%d)' % (
        ...
        for id_attack, attack in enumerate(attacks):
            ...
            for k in range(len(threshold)):
                ...
                with torch.no_grad():
                    ...
                    mixture_output[k][id_attack][batch_idx] = predictions[k][id_attack][batch_idx][0].eq(
                        targets).int() * model.weights[0] + predictions[k][id_attack][batch_idx][1].eq(targets).int() * \
                                                              model.weights[1]

        accuracy = mixture_output[:, :, :batch_idx + 1, :].min(1)[0].sum(dim=1).sum(dim=1) / total
        for att in range(len(attacks)):
            print("From the " + str(att) + "th attack, for each threshold : ",
                  (mixture_output[:, :, :batch_idx + 1, :].min(1)[1].eq(att).sum(dim=1).sum(dim=1) * 100) / total)
        print(accuracy)
